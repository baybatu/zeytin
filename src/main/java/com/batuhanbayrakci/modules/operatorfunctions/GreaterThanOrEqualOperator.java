...
public class GreaterThanOrEqualOperator implements ZyOperatorFunction {
    ...
    public void process(ZyStack stack) throws ZyStackUnderflowError, ZyTypeError {
        ...
        if (!secondObject.getClass().isAssignableFrom(firstObject.getClass())) {
        ...
        if (secondObject instanceof ZyNumber) {
            boolean result = (Double) secondObject.getValue() >= (Double) firstObject.getValue();
            stack.push(new ZyBoolean(result));
        } else {
